...
import MLGenerator.Html;
import MLGenerator.Xml;
...
public class IDECompiler {
    ...
    public boolean compileProgram(String sourceName) {
        System.out.println("********** " +
                           "Triangle Extended Compiler (IDE-Triangle 1.0)" +
                           " **********");
        SourceFile sourceHtml = new SourceFile(sourceName);
        Scanner scannerHtml = new Scanner(sourceHtml);
        Html fileWriter = new Html(sourceName, scannerHtml);
        boolean hasLexicErrors = fileWriter.checkLexic();
        
        if (!hasLexicErrors) {
            fileWriter.write(fileWriter.getBuffer());
        }else{
            System.out.println("Lexic Analysis Failed...");
        }
        ...
        if (success){
            Xml xmlWriter = new Xml(sourceName);
